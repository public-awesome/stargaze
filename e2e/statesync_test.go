...
func TestStateSync(t *testing.T) {
	...
	latestHeight, err := stargazeChain.Height(ctx)
	...
	firstFullNode := stargazeChain.FullNodes[0]
	...
	require.NoError(t, stargazeChain.AddFullNodes(ctx, configFileOverrides, 1))

	// Wait for new node to be in sync.
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	require.NoError(t, testutil.WaitForInSync(ctx, stargazeChain, stargazeChain.FullNodes[len(stargazeChain.FullNodes)-1]))
