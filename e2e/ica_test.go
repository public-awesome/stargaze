...
func TestInterchainAccounts(t *testing.T) {
	...
	cf := interchaintest.NewBuiltinChainFactory(zaptest.NewLogger(t), []*interchaintest.ChainSpec{
		{
			...
			ChainConfig: ibc.ChainConfig{
				Images:                 []ibc.DockerImage{{Repository: "ghcr.io/cosmos/ibc-go-icad", Version: "v0.2.6"}},
	...
	_, err = cosmos.PollForMessage(ctx, chain2, ir, c2h, c2h+60, channelFound)
	require.NoError(t, err)
	// stdout, _, err = chain2.Exec(ctx, channels, nil)
	// require.NoError(t, err)
	// t.Log(string(stdout))
	...
	txHash, err := chain1.Validators[0].RegisterICA(ctx, chain1User.KeyName(), connections[0].ID)
	require.NoError(t, err)
	t.Log(txHash)
