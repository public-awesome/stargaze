...
func TestMinDepositDecorator(t *testing.T) {
	...
	params := app.GovKeeper.GetParams(ctx)
	params.MinDeposit = sdk.NewCoins(sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(5_000_000_000)))
	app.GovKeeper.SetParams(ctx, params)
	app.Commit()
	content, found := legacygovtypes.ContentFromProposalType("Prop Title", "Description", legacygovtypes.ProposalTypeText)
	require.True(t, found)

	createProposalMsg, err := legacygovtypes.NewMsgSubmitProposal(content, sdk.NewCoins(bondCoin), addr1)
	...
	createProposalMsg, err = legacygovtypes.NewMsgSubmitProposal(content, sdk.NewCoins(sdk.NewInt64Coin(sdk.DefaultBondDenom, 1_000_000_000)), addr1)
	require.NoError(t, err)
	...
	createProposalMsg, err = legacygovtypes.NewMsgSubmitProposal(content, sdk.NewCoins(sdk.NewInt64Coin(sdk.DefaultBondDenom, 4_000_000_000)), addr2)
	require.NoError(t, err)
	header = tmproto.Header{Height: app.LastBlockHeight() + 1}
