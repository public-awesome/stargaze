...
func TestPost_EmptyCreator(t *testing.T) {
	...
	addrs := simapp.AddTestAddrsIncremental(app, ctx, 3, sdk.NewInt(1000000))
	err = app.CuratingKeeper.CreatePost(ctx, vendorID, postID, bodyHash, body, nil, addrs[1], "", nil, "", nil)
	require.NoError(t, err)
...
func TestPost_EmptyRewardAccount(t *testing.T) {
	...
	err = app.CuratingKeeper.CreatePost(ctx, vendorID, postID, bodyHash, body, addrs[0], nil, "", nil, "", nil)
	require.NoError(t, err)
...
func TestPost_WithRewardAccount(t *testing.T) {
	...
	err = app.CuratingKeeper.CreatePost(ctx, vendorID, postID, bodyHash, body, addrs[0], addrs[1], "", nil, "", nil)
	require.NoError(t, err)
