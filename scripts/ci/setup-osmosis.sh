...
sed -i 's/localhost:9090/0.0.0.0:9090/g' ~/.osmosisd/config/app.toml
sed -i -e 's/timeout_commit = "5s"/timeout_commit = "100ms"/g' ~/.osmosisd/config/config.toml
sed -i -e 's/timeout_propose = "3s"/timeout_propose = "100ms"/g' ~/.osmosisd/config/config.toml
