...
icad tx intertx register --from $ICA_WALLET_ADDRESS --connection-id connection-0 --chain-id ica --keyring-backend test  --generate-only >  tx.json  
cat tx.json | jq
icad tx sign tx.json --from $ICA_WALLET_ADDRESS --chain-id ica --keyring-backend test 
